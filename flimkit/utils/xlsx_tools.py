...
def _csv_layout(path: str | Path) -> tuple[str, int]:
    ...
    raise ValueError(
        f'Could not parse {Path(path).name}: no tab, comma or semicolon '
        'delimited header row containing "Time [" was found.'
    )
...
def _read_export(path: str | Path, header=None) -> pd.DataFrame:
    ...
    if suffix in ('.csv', '.tsv', '.txt', '.dat', '.ascii', '.asc'):
    ...
    raise ValueError(
        f'Unsupported IRF export format: {suffix or "<none>"}. '
        'Expected .xlsx, or a delimited text export '
        '(.csv, .tsv, .txt, .dat, .ascii, .asc).'
    )
