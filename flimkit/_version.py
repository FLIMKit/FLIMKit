...
0.11.0 -> a bridge to QuPath: FLIMKit serves its intensity and lifetime images and its ROIs over a loopback HTTP connection, so QuPath reads and writes them directly instead of going through exported files, against the image already open in a live session, and can put the FLIMKit images into the open project beside a brightfield image. Plugins gain two hooks: @startup(id, order) runs a callback once the window is built, for an add-on that has to be doing something from launch, and @panel_button(id, label, panel, order) puts a button in the ROI panel's action grid. A startup that raises is reported and the rest still run. Freehand ROIs whose outline crosses itself no longer export as self-intersecting GeoJSON, which RFC 7946 does not allow and which made QuPath reject a whole FeatureCollection over one bad region; such a ring is repaired to its outer boundary and carries repaired: self-intersecting. Three of five hand-drawn regions in a real session were affected. Adds shapely as a dependency.
0.12.0 -> on PyPI, so pip install flimkit gets the analysis core without the desktop, with gui, torch, mlx, notebook, segmentation and test as extras. Plugins now load when a format is looked up, so a plugin-registered format works from a script or a CLI and not only from the GUI. Per-pixel fitting on the GPU works in pixel blocks, so a field larger than a gigabyte reaches the GPU instead of silently falling back to the CPU; a 1024 square field fits in 7.8s against 25.2s on the CPU. The per-pixel tau grid goes from 200 points to 1600, which cuts the error against known-truth synthetic data from 0.021 ns to 0.008 ns, so per-pixel lifetimes will differ slightly from earlier versions. Cancelling a tile fit is honoured through canvas assembly and saving rather than only during the fitting. Leica .xlef projects are read for tile positions.
0.13.0 -> per-pixel fitting numbers move, so fitter_version goes to 20. Free-tau per-pixel fits carry their lifetimes in nanoseconds rather than seconds, which roughly halves the error against known truth and makes the CPU, CUDA and Metal paths agree to every digit reported. Pile-up correction was silently discarded on the free-tau path and now runs, and pile-up can instead go in the per-pixel forward model with --pileup-in-model, which leaves the data Poisson. The background can be fitted as a term in the model rather than subtracted, with --bg-in-model, off by default because the two routes are biased in opposite directions rather than one being better. Per-pixel chi-squared is computed in float32, which takes a 512 square fit from 2.02s to 1.50s and the test suite from 280s to 155s. The GPU block budget is per backend, 32 MB on CUDA and ROCm against 256 MB on MLX, worth a flat 2x on an A5000, and is clamped to half of free device memory.
