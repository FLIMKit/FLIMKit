...
### Added
- Public plugin bindings for the current intensity and lifetime images and for GeoJSON ROI import and export. `get_current_images`, `export_rois_geojson` and `import_rois_geojson` hide private GUI fields, return image copies, refresh ROI views after import and safely marshal background plugin calls to the GUI thread.
