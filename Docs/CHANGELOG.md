...
### Added
- `bg_in_model` fits the background as a term in the model instead of subtracting an estimate before projecting, on the one-exponential scan and the fixed-tau path. The one-exponential case centres the data and the basis, since projecting out a constant is mean subtraction; the fixed-tau case adds a column of ones to the NNLS design, which is what the time-varying background path already did. The two routes are biased in opposite directions rather than one being better: subtracting reads low, modelling reads high, and modelling degrades sharply when the window holds fewer than about five lifetimes, because the offset and the unresolved tail trade off. Measured on a known 2.4 ns decay with 40 counts per bin of background: -0.02 ns subtracting against +0.05 ns modelling over 5.4 lifetimes, and -0.12 ns against +0.29 ns over 3.2. The default does not change. `--bg-in-model` on the CLI and both interactive parsers, and a checkbox in Expert Fit Settings. It runs on the CPU, and says so when a GPU backend is available.
...
- Free-tau per-pixel fitting on a GPU backend no longer runs its scipy fits through a thread pool. The pool was sized to the core count and every fit holds the GIL, so it only added contention: on an A5000 with 24 cores, one worker took 8.28s and 48 workers took 15.56s on the same 2304 pixels, with the answers identical at every thread count. A plain loop now runs them, which makes the GPU route 1.9x faster and slightly faster than the CPU path it was losing to. No fitted number changes.
- Per-pixel chi-squared is computed in float32. A profile of a 512 square fit put `_scatter_1exp` at 45 per cent of the run and `chi2_terms` alone at 34 per cent, because it forced both the data and the model to float64 and built a 962 MB intermediate where 481 MB would do. The fit goes from 2.02 s to 1.50 s, and the whole test suite from 280 s to 155 s. Measured over 65,536 pixels the worst relative difference is 1.6e-07 on the chi-squared numerator and 3.4e-07 on the calibrated value, with the validity masks identical. `chi2_terms` still defaults to float64, so the summed fits are untouched; only the per-pixel scatter asks for float32.
...
### Fixed
- Free-tau per-pixel fits carry the lifetimes in nanoseconds rather than seconds. The parameter vector held lifetimes around 1e-9 next to amplitudes around 1e2, nine orders apart, and SciPy's trust region solver stalled on it: on a heavily piled-up decay it stopped after 11 function evaluations at a cost of 81 where 21 evaluations reach 43. Against a known-truth bi-exponential the error roughly halves, RMSE 0.0873 ns to 0.0415 ns at 60,000 photons and 0.2110 ns to 0.1520 ns at 6,000. Two things that had looked like separate problems went away with it: the CPU and GPU paths now return identical lifetimes, where they used to differ by 0.037 ns at low photon counts, and CUDA and Metal agree to every digit reported. Every free-tau per-pixel lifetime moves slightly, which is the other half of `fitter_version` 20.
...
- A bridge to QuPath, documented under QuPath Bridge. FLIMKit serves its intensity and lifetime images and its ROIs over a loopback HTTP connection, and QuPath reads and writes them directly instead of going through exported files. It runs inside a live QuPath session, so it works with the image you have open, and it can put the FLIMKit images into the open project to sit beside a brightfield image in the viewer.
