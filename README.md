...
- **Time-resolved anisotropy**: explicit parallel/perpendicular PTU pairs, configurable G factor and relative exposure, subpixel registration, overlapping pooled maps, photon masks, and CSV/NPZ export
...
For paired polarization acquisitions, open **Tools → Time-Resolved Anisotropy** and assign the parallel and perpendicular PTUs explicitly. **Direct r(t) diagnostic** keeps the background-corrected trace and registered neighborhood-pooled maps. **Preferred global fit** follows Lakowicz Section 11.2.2: it jointly fits the raw polarized count decays with separate measured IRFs, a known fluorescence lifetime, fixed G and relative exposures, periodic excitation, one rotational correlation time, a common IRF shift, and separate backgrounds. G=1 is only an assumption unless calibrated. Inspect residuals and bound warnings; the first model does not provide calibrated uncertainty or support multiple rotational components.
